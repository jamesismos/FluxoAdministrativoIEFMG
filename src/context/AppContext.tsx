...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      const storedProcesses = localStorage.getItem('narflow_processes');
      const storedAsv = localStorage.getItem('narflow_asv');
      const storedTaxas = localStorage.getItem('narflow_taxas');
      const storedNormatives = localStorage.getItem('narflow_normatives');
      const storedSettings = localStorage.getItem('narflow_settings');
  ...
  useEffect(() => {
    if (!isLoaded) return;
    localStorage.setItem('narflow_processes', JSON.stringify(processes));
  }, [processes, isLoaded]);

  useEffect(() => {
    if (!isLoaded) return;
    localStorage.setItem('narflow_asv', JSON.stringify(asvRecords));
  }, [asvRecords, isLoaded]);

  useEffect(() => {
    if (!isLoaded) return;
    localStorage.setItem('narflow_taxas', JSON.stringify(taxaRecords));
  }, [taxaRecords, isLoaded]);

  useEffect(() => {
    if (!isLoaded) return;
    localStorage.setItem('narflow_normatives', JSON.stringify(normatives));
  }, [normatives, isLoaded]);

  useEffect(() => {
    if (!isLoaded) return;
    localStorage.setItem('narflow_settings', JSON.stringify(settings));
  }, [settings, isLoaded]);
